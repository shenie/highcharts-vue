...
const title = ref(''),
  ...
  chartType = ref('Spline'),
  seriesColor = ref('#6fcd98'),
  colorInputIsSupported = ref(null),
  animationDuration = ref(1000),
  updateArgs = ref([true, true, { duration: 1000 }]);
...
watch(points, newPoints => {
  chartOptions.value.series[0].data = newPoints;
}, { deep: true });

watch(chartType, newChartType => {
  chartOptions.value.chart.type = newChartType.toLowerCase();
});

watch(seriesColor, newSeriesColor => {
  chartOptions.value.series[0].color = newSeriesColor.toLowerCase();
});

watch(animationDuration, newAnimationDuration => {
  updateArgs.value[2].duration = Number(newAnimationDuration);
});

const i = document.createElement('input');
i.setAttribute('type', 'color');

colorInputIsSupported.value = (i.type === 'color');
